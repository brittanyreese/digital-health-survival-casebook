...
| 10_churn_ml.py | HistGradientBoosting, 5-fold stratified CV (single-loop, not nested), SHAP | ML / XAI |
...
| 12_recovery_monte_carlo.py | Monte-Carlo recovery intervals across 25 independent seeds | Recovery-stability check (slow; run once, committed, off the CI hot path) |
| 13_misspecification_recovery.py | Non-Weibull baselines + Cox-Snell goodness-of-fit | Baseline-family adequacy check (regenerates its own cohort) |
...
Schoenfeld (1983) formula: events = 4(z*α+z*β)²/(ln HR)²
