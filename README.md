...
A methods casebook for digital-health survival analysis, built on a **fully
simulated** world: the Acme quit-support program, a smoking-cessation app from
Acme Health, a fictional vendor. No real participant data exists in this
project; all organizations, programs, and people are fictitious, and any
resemblance to real studies or products is coincidental. Every dataset is
generated by a committed, seeded simulation whose parameters are drawn from
published literature and public federal data (NHANES, MMWR), so every analysis
can be scored against the values that were injected rather than argued from
authority.

The pipeline recovers those injected parameters, and that is the whole claim.
The results show the analysis code returns what the generator put in. They are
...
correct censoring handling, and ML-based early-warning prediction.  It runs
on the synthetic Acme quit-support cohort because the pipeline, not the data,
is the contribution.
...
Under seed 42 the result CSV tables are byte-identical across runs. Figures
are not held to that standard: one SHAP summary plot
...
narrower than MLE; treat those as directional only.  The Schoenfeld residual test
...
subjects with early relapses have mechanically fewer events (their event
count is bounded by how long they survived), creating a structural positive correlation between
...
pre-landmark; that monotonic pattern was the bias signature.  On
...
estimates generalization; the full-data model maximizes explanation signal).
